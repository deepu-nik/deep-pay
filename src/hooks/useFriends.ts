...
export function useFriends() {
  ...
  const refresh = useCallback(async () => { setLoading(true); setFriends(await mockFriendService.listFriends()); setLoading(false); }, []);
  const addFriend = useCallback(async (name: string) => { const friend = await mockFriendService.addFriend(name); setFriends((current) => [...current, friend]); return friend; }, []);
